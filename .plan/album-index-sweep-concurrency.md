...
`sweep_stale_asset_paths` (`backend/src/tasks/actor/album_index.rs`) runs at the end of a manual album index. Two
concerns:
...
2. **Detached-batch ordering race.** The per-file index tasks enqueue `FlushTreeTask` inserts as detached batches;
   `sweep_stale_asset_paths` removes/ updates records via `execute_batch_detached`; both are drained afterwards with
...
- [ ] Sweep cascades thumbnail deletion when a record's canonical path vanishes (`compressed_path` remove; shared
      thumbnails are preserved via `DUPE_INDEX`) — confirm that is the intended single-canonical-path behavior (a record
      whose path is gone is removed in full; there is no partial prune), with a test if missing.

## Progress (2026-09-08)

Fixed in PR \#17 (`album_index.rs`): the walk's `FlushTreeTask` inserts are now drained with `execute_batch_waiting`
BEFORE `sweep_stale_asset_paths`, so the sweep reads a current in-memory tree and the later drain cannot re-apply a record
...
cascade behavior when a record's canonical path vanishes (third task) was confirmed unchanged but is not directly
tested.
