...
// `abstractData.alias.file` (singular). There is no `currentAlias` and no
// row-level `hash` — see frontend/src/type/schemas.ts (AliasSchema,
// databaseTimestampSchema) and backend scenario
// locate_same_hash_by_asset_id.yaml (response.json.[0].assetId /
// response.json.[0].abstractData.alias.file).
function mediaRow(assetId: string, file: string) {
  return {
    ...
    abstractData: {
      ...
      alias: { file, modified: 0, scanTime: 0, isTrashed: false }
    }
  }
}

describe('findPhotoAssetId', () => {
  it('returns the row assetId whose alias.file ends with the qualified path', () => {
    const data = [
